...
\end{enumerate}
色クラスの特徴空間を図\ref{fig:DistributionColors}に示す．それぞれのクラス感距離を表\ref{fig:color_class_distance}に示す.
また，図\ref{fig:ColoredEMNIST}に各クラスの画像例の一覧を示す．
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Colored EMNISTデータセットにおける色概念の難易度調整例．各図は異なる$\sigma^2$値で生成されたサンプルを示している．$\sigma^2$が増加するにつれて，色概念のばらつきが大きくなる．}
    \label{fig:coloredeminsts_2x2}
\end{figure}
